...
export default function Navbar() {
  ...
  <img src="images/logo.png" alt="AJ Digital Growth Logo" style="height: 40px;">
        ...
        <button className="flex md:hidden flex-col gap-[5px] cursor-pointer bg-none border-none p-[4px]" id="hamburger" aria-label="Menu" onClick={toggleMenu}>
          <span className="block w-[24px] height-[2px] bg-white transition-all duration-300 rounded-[2px]"></span>
          <span className="block w-[24px] height-[2px] bg-white transition-all duration-300 rounded-[2px]"></span>
          <span className="block w-[24px] height-[2px] bg-white transition-all duration-300 rounded-[2px]"></span>
        </button>
      </div>
      <div className={`${isOpen ? 'flex' : 'hidden'} fixed top-[64px] left-0 right-0 bg-[#1B2A4A] z-[999] p-[1.5rem] flex-col gap-[1rem] border-b-2 border-[#F5A623]`} id="mobileMenu">
        <a href="#problem" className="text-[16px] color-white font-medium py-[.5rem]" onClick={() => setIsOpen(false)}>The Problem</a>
        <a href="#services" className="text-[16px] color-white font-medium py-[.5rem]" onClick={() => setIsOpen(false)}>Services</a>
        <a href="#about" className="text-[16px] color-white font-medium py-[.5rem]" onClick={() => setIsOpen(false)}>About AJ</a>
        <a href="#pricing" className="text-[16px] color-white font-medium py-[.5rem]" onClick={() => setIsOpen(false)}>Pricing</a>
        <a href="#contact" className="text-[16px] color-white font-medium py-[.5rem]" onClick={() => setIsOpen(false)}>Contact</a>
        <a className="btn btn-primary justify-center mt-[.5rem]" href="https://wa.me/265987808110?text=Hi%20AJ%2C%20I%20want%20a%20free%20audit" target="_blank" rel="noreferrer">Get Free Audit →</a>
      </div>
    </nav>
  );
}
